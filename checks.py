...
Script uses Boolean values to capture the success/failure of a check. it also employs SSH key to be added automatically as the script runs through a chunk of hosts
...
[root@srv0001 python]# ./checks.py ips.txt
Host is up & running
Warning: Permanently added '172.16.46.90' (RSA) to the list of known hosts.
Puppet agent is running
Sysedge agent is running
Trap destinations are fine
172.16.46.90 is in Smarts
['qa1654', True, True, True, True, True, ['STRING: "crond"', 'STRING: "ntpd"', 'STRING: "sshd"', 'STRING: "syslogd"', 'STRING: "cron"'], (True, '31')]
[root@srv0001 python]# tail -1 stats.csv
qa1654,True,True,True,True,True,"['STRING: ""crond""', 'STRING: ""ntpd""', 'STRING: ""sshd""', 'STRING: ""syslogd""', 'STRING: ""cron""']","(True, '31')"
...
def puppetagent(ip):
    ...
    cmd2 = 'ssh -i /home/hitesha/id_dsa -o StrictHostKeyChecking=no ' + ip + ' "service puppet status"'
    try:
...
def sysedgeagent(ip):
    ...
    cmd3 = 'ssh -i /home/hitesha/id_dsa -o StrictHostKeyChecking=no ' + ip + ' "service sysedge status"'
...
def sysedgeconfig(ip):
    ...
    cmd4 = 'ssh -i /home/hitesha/id_dsa -o StrictHostKeyChecking=no ' + ip + ' "grep trap_community /etc/sysedge.cf | grep public"'
